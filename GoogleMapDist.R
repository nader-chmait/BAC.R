...
for (index in seq(0, nrow(club.distance.to.cbd), by=step)){
  ...
  #print(paste(i, j, sep = "-"))
  result[i] <- getDistToCBD(club.distance.to.cbd$Club[i:j], club.distance.to.cbd$CBD.query[i:j])
}
...
club.distance.to.cbd$club.distance.to.cbd <- as.integer(club.distance.to.cbd$DistToCBD)
club.distance.to.cbd$RegionLocation <- as.character("Remote area")
club.distance.to.cbd[club.distance.to.cbd$DistToCBD<= 5 , "RegionLocation"] <- "Inner City"
club.distance.to.cbd[club.distance.to.cbd$DistToCBD>= 6  & club.distance.to.cbd$DistToCBD< 11 , "RegionLocation"] <- "City"
club.distance.to.cbd[club.distance.to.cbd$DistToCBD>= 11 & club.distance.to.cbd$DistToCBD< 21 , "RegionLocation"] <- "City Border"
club.distance.to.cbd[club.distance.to.cbd$DistToCBD>= 21 & club.distance.to.cbd$DistToCBD< 40 , "RegionLocation"] <- "Suburb"
club.distance.to.cbd[club.distance.to.cbd$DistToCBD>= 41 & club.distance.to.cbd$DistToCBD< 80 , "RegionLocation"] <- "Outer Suburb"

club.distance.to.cbd$RegionLocation <- as.factor(club.distance.to.cbd$RegionLocation)

#write.csv(club.distance.to.cbd, "club.distance.to.cbd.csv")
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------
